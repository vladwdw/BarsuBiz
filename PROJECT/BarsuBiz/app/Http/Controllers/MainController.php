...
class MainController extends Controller
{
    ...
    {  try{
        if($name=="Молодежные инициативы"){
            $result=new StoreController1;
            $filePath=$result->form11_word($id,$name);
            return response()->download($filePath)->deleteFileAfterSend();
        }
        if($name=="РКИП")
        {
        $Rcipword=new Form6WordController;
        $zip_file=$Rcipword->form_word($id,$name);
        ...
        if($name=="Участие в НИР")
        {
          $nirWord=new StoreController2;
            $filePath=$nirWord->form_word($id,$name);
        return response()->download($filePath)->deleteFileAfterSend();

        }
        if($name=="100 ИДЕЙ ДЛЯ БЕЛАРУСИ"){
            $hundredIdeasWord=new StoreController3;
            $newFileName=$hundredIdeasWord->form_word($id,$name);
            return response()->download($newFileName.'.docx')->deleteFileAfterSend();
        }
        if($name=="ГПНИ"){

                $gpniWord=new Form4WordController;
                $zip_file=$gpniWord->form_word($id,$name);
        return response()->download($zip_file)->deleteFileAfterSend();
        }
        if($name=="Заявка на получение гранта"){
   
            $grantWord=new Form5WordController;
            $zip_file=$grantWord->form_word($id,$name);        
            return response()->download($zip_file)->deleteFileAfterSend();
        }
    }
    catch (\Exception $e) {
